...
#exploraing the header and summary to see the variables and descriptive statistics demo_train 
head(demo_train)
summary(demo_train)

#exploraing the header and summary to see the variables and descriptive statistics in LU_train 
head(LU_train)
summary(LU_train)

#exploraing the header and summary to see the variables and descriptive statistics in token_train 
head(token_train)
summary(token_train)
...
LU_train_sub <-
  select(LU_train, SUBJ, VISIT, CHI_MLU, MOT_MLU)
...
write.csv(df, file = "/CogSci/Semester 3/Experimental Methods 3/ExpMeth3Ny/methods3_A1-master/clean_data_A1.csv")
...
```{r}
sub_df1 <- df %>% group_by(ID) %>% filter(mean(CHI_MLU) > 2.7)

sub_df2 <- df %>% filter(VISIT == 1 & CHI_MLU < 1.5)

sub_df3 <- df %>% group_by(ID) %>% filter(sum(VISIT) != 21)
...
```{r}
token6 <-
  df %>% filter(VISIT == 6) %>% arrange(-tokens_CHI)

token6[1, 1]

token1 <-
  df %>% filter(VISIT == 1) %>% arrange(tokens_CHI)
...
```{r}
sub_ASD <-
  df %>% filter(Diagnosis == 'ASD') %>% select(CHI_MLU, tokens_CHI)

sub_ASD2 <-
  df %>% filter(Diagnosis == 'ASD') %>% select(CHI_MLU, tokens_CHI, tokens_CHI)
...
```{r}
# 1
df1 <- df %>% group_by(ID) %>% mutate(m_tokens_CHI = mean(tokens_CHI))

# 3
# The children develop over time. We would not be able to explore this change if we only looked at the averages.
