...
#This part of the code is written by PB
```{r}
...
# Exploring the header and summary to see the variables and descriptive statistics demo_train 
head(demo_train)
summary(demo_train)

# Exploring the header and summary to see the variables and descriptive statistics in LU_train 
head(LU_train)
summary(LU_train)

# Exploring the header and summary to see the variables and descriptive statistics in token_train 
head(token_train)
summary(token_train)
...
# Renaming the column names of subject ID and visit nr., so it is consistent throughout all three datasets 
...
# This part of the code is written by SOH
...
# This part of the code is written by JJ
...
# Renaming a misspelling in the Ethnicity variable for participant nr 6, visit 3
df[32,4] <- "Bangladeshi"
...
# This part of the code is written by KK

```{r}

# Wishing to only keep the "visit 1" scores in the four clinical variables of the ADOS score, the verbal IQ, the nonverbal IQ and the socialization score, a subset is created where those four, subject ID is included only for the first visit for each participant. A column with a number ID is also created while there is just one column for each participant, which can then be used as identifier instead of the "SUBJ" variable.   
data_sub <- df %>% 
...
```{r}
# Substituting the numbers in the Gender variable for F for females and M for males as well as making the variable a factor
df$Gender <- ifelse(df$Gender=="1","M","F")
df$Gender <- as.factor(df$Gender)

# Substituting the letters in the Diagnosis variable for ASD instead of A and TD instead of B as well as making the variable a factor
df$Diagnosis <- ifelse(df$Diagnosis=="A","ASD","TD")
df$Diagnosis <- as.factor(df$Diagnosis)

# Write the dataframe into a csv file and save it
#write.csv(df, "cleanedData.csv",row.names = FALSE) #commented out, because of different computers
...
ggplot(df, aes(x = Socialization, y = CHI_MLU, col = VISIT)) + 
  ...
  ggtitle("Child MLU as a function of Socialization") # There looks to be a correlation between sociability and MLU
...
# This part of the code is written by CP
...
```{r}
# Listing all children that have a MLU across visits of more than 2.7 morphemes 
df_sub <- df %>% 
  group_by(ID)  %>% # Grouping the data by ID to ensure that the mean is taken for each child
  filter(mean(CHI_MLU) > 2.7) # Filter by mean length of utterances

View(df)
# Listing all children who hava a MLU less than 1.5 for the first visit
filter(df,CHI_MLU < 1.5 & df$VISIT == 1)

# Listing the children who have not completed all trials 
df_sub2 <- df %>% 
  group_by(ID)  %>% 
  filter(NROW(VISIT)<6) # Filter by how many visits the children has had by counting the number of rows for each child and only include those where the count is less than 6
```
...
# Sort the dataset according to who produced the most words on the 6th visit
df_sub3 <- df %>% 
  filter(VISIT==6) %>% 
  arrange(desc(tokens_CHI))

# Show the ID of the child who produced the most words
df_sub3[1,1]


# Sort the dataset according to who produced the least amount of words on the first visit
df_sub4 <- df %>% 
  filter(VISIT==1) %>% 
  arrange(tokens_CHI)

# Show the ID of the child who produced the least words 
df_sub4[1,1]
...
```{r}
# Make the ID a character
df$ID <- as.character(df$ID)

# Making a subset of the data with ID, MLU and number of words, only for children with ASD
df_sub5 <- df %>% 
  filter(Diagnosis == "ASD") %>% 
  select(c(ID,Diagnosis,CHI_MLU,tokens_CHI))

# Testing what happens if a variable is included twice in the select() function
select(df,ID,Diagnosis,Diagnosis,CHI_MLU)
#The variable is only included once
```
...
# Adding a column with the average amount of words for each child across all visits 
df_ext <- df %>% 
  group_by(ID) %>% 
  mutate(m_token_CHI = mean(tokens_CHI)) 
