...
token_train = read_csv('Data/token_train.csv')
```
...
df1 <- merge(demo_train_sub, LU_train_sub, all = T)
df <- merge(df1, token_train_sub, all = T)
...
write.csv(df, "Data/cleanedData.csv", row.names = FALSE)
...
ggplot(df, aes(x = Socialization, y = CHI_MLU, col = VISIT)) +
  ...
  ggtitle("Child MLU as a function of Socialization") # There seems to be a correlation between sociability and MLU
